...
Fixou-se em 5% a retirada de dados do total observações por individuo, a fim de que não perdesse informações relevantes ao problema.
...
```{r graf_mae_1 ,echo=FALSE}

# boxplot do peso do bebe por raça da mãe
g1 = ggplot(data = dados, aes(x = factor(race),y=wt))+geom_boxplot()+theme_classic()

# boxplot do peso do bebe por nivel educacional da mãe
g2 = ggplot(data = dados, aes(x = factor(ed),y=wt))+geom_boxplot()+theme_classic()

g1mae = grid.arrange(g1, g2, ncol=1,nrow=2)
...
```{r graf_mae_2,echo=FALSE}
oipi = which(grepl(99,dados$mht))
dado = dados[-oipi,]

g3 = ggplot(data = dado, aes(x = mht,y=wt))+geom_point()+theme_classic()

oipi = which(grepl(999,dados$mwt))
dado = dados[-oipi,]

# boxplot do peso do bebe por nivel educacional da mãe
g4 = ggplot(data = dado, aes(x = mwt,y=wt))+geom_point()+theme_classic()

g2mae = grid.arrange(g3, g4, ncol=1,nrow=2)
...
13. mwt - peso pré-gradidez da mãe em libras, 999 = desconhecido
...
19. marital - 1 = casados, 2 = separados legalmente, 3 = 
